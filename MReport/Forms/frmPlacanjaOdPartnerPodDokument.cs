...
{
    public partial class frmPlacanjaOdPartnerPodDokument : frmIzvestaj
    {
        public frmPlacanjaOdPartnerPodDokument()
        {
            InitializeComponent();
        }
        ...
        private void btnPotvrdi1_Click(object sender, EventArgs e)
        {
            ...
            viewDokumentiZaPlakjanjeTableAdapter.Fill(bSSRDataSet.viewDokumentiZaPlakjanje,Tools.PrazenStringToInt(zbxPartner.ret1), new DateTime(datumOd.DateTime.Year,
        ...
        public void presmetaj()
        {
            ...
            sumaText = string.Format("Должи: {0} Побарува: {1} " + Environment.NewLine + " Салдо (Д-П): {2}",
                Tools.PrazenStringToDecimal(bSSRDataSet.viewDokumentiZaPlakjanje.Compute("SUM(DolziPoDokument)", "")
                    .ToString()).ToString("N2"),
                Tools.PrazenStringToDecimal(bSSRDataSet.viewDokumentiZaPlakjanje.Compute("SUM(PlatenoPoIzvod)", "")
                    .ToString()).ToString("N2"),
                Tools.PrazenStringToDecimal(bSSRDataSet.viewDokumentiZaPlakjanje
                    .Compute("SUM(MomentalnoSaldoNaDokument)", "").ToString()).ToString("N2"));
