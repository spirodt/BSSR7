...
{
    public partial class frmPltObrazec : frmIzvestaj
    {
        public frmPltObrazec()
        {
            InitializeComponent();
        }
        ...
        private void btnPecati1_Click(object sender, EventArgs e)
        {
            ...
            var middleColumn = String.Format("Приемен лист во трговија на мало: {0}  до: {1} ", datumOd.DateTime.ToShortDateString(), DatumDo.DateTime.ToShortDateString());
        ...
        private void btnPotvrdi2_Click(object sender, EventArgs e)
        {
            if(rbtnPocetok.Checked)
                pLTobrazecTableAdapter.FillByBrojPriem(materijalnoDataSet.PLTobrazec, PriemOD.PrazenStringToInt(), PriemDO.PrazenStringToInt());
            if(rbtnKraj.Checked)
                pLTobrazecTableAdapter.FillByBrojDokument(materijalnoDataSet.PLTobrazec, PriemOD.PrazenStringToInt(), PriemDO.PrazenStringToInt());
        }

        private void btnPotvrdi1_Click(object sender, EventArgs e)
        {
            if (rbtnPocetok.Checked)
            pLTobrazecTableAdapter.FillByPriemDatum(materijalnoDataSet.PLTobrazec, new DateTime(datumOd.DateTime.Year,
                                                                                                                  ...
                                                                                                                  DateTimeKind.Local));
            if (rbtnKraj.Checked)
                pLTobrazecTableAdapter.FillByDokumentDatum(materijalnoDataSet.PLTobrazec, new DateTime(datumOd.DateTime.Year,
                                                                                                                      datumOd.DateTime.Month,
                                                                                                                      datumOd.DateTime.Day,
                                                                                                                      00,
                                                                                                                      00,
