CREATE FUNCTION [dbo].[PregledNaSiteSmetki]
(
...
BEGIN
	...
	(
	 ...
	 DatumPocetok AS Datum,
	 CONCAT (
	    (	SELECT TOP 1 value FROM STRING_SPLIT(p.EDB, '-')),
			CONVERT(NVARCHAR(50), format(dbo.viewSmetki.DatumPocetok,'ddMMyyyyHHmm')),
			RIGHT('000000'+CONVERT(NVARCHAR(50),
			ISNULL(CAST(ROUND(SUM(ISNULL(Iznos,0)),0) AS DECIMAL(18,2)),0)
			, 128), 6)
		) as Rezultat
	 FROM dbo.viewSmetki
	...
	GROUP BY p.Naziv,PresmetkovnaEdinica,EDB,DatumPocetok
